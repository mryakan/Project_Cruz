...
"""
notMNist utils for loading & pickling images from:
...
def find_folders(train_foldername, test_foldername):
    ...
    for adir in [train_dir, test_dir]:
        if not os.path.exists(adir) or not os.path.isdir(adir):
            print("ERROR: Invalid Directory '%s'" % adir)
            return None, None
    train_folders = [os.path.join(train_dir, d) for d in os.listdir(train_dir) if not d.endswith(".pickle")]
